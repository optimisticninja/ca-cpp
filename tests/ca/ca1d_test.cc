...
TEST(CA1D3P, PartitionSizeAndValues)
{
    ...
    GatewayKey<PARTITION_SIZE, bool> gateway_key(start_state, BOUNDARY_ZERO, PARTITION_BIAS_LHS);
...
TEST(CA1D4P, RHSBiasing)
{
    ...
    vector<bool> start_state = wolfram_start_state(state_size);
    GatewayKey<4, bool> gateway_key(start_state, BOUNDARY_ZERO, PARTITION_BIAS_RHS);
...
TEST(CA1D4P, LHSBiasing)
{
    size_t state_size = 31;
    vector<bool> start_state = wolfram_start_state(state_size);
    GatewayKey<4, bool> gateway_key(start_state, BOUNDARY_ZERO, PARTITION_BIAS_LHS);
    CA1D<4, bool> ca(gateway_key);
    vector<bool> partition = ca.partition(0);
